...
| Fresh foreground Apple ID preauthentication before release effects, optional explicit API-key mode, and version-bound TestFlight completion checkpoints | `SPEC-RWKV-APPLE-RELEASE-AUTH-GATE` | active | `docs/contracts/app_distribution.md` | `.env.example`, `fastlane/Fastfile`, `fastlane/README.md`, `tools/fastlane/apple_auth_gate.rb`, `tools/fastlane/apple_auth_gate_test.rb` |
