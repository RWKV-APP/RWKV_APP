...
| Non-interactive TestFlight authentication, explicit one-attempt Apple ID fallback, and version-bound completion checkpoints | `SPEC-RWKV-APPLE-RELEASE-AUTH-GATE` | active | `docs/contracts/app_distribution.md` | `.env.example`, `fastlane/Fastfile`, `fastlane/README.md`, `tools/fastlane/apple_auth_gate.rb`, `tools/fastlane/apple_auth_gate_test.rb` |
