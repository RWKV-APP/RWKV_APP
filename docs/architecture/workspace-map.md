...
1. Identify whether the issue is frontend, adapter, engine, or website
2. Inspect the narrowest call chain before editing
3. Prefer repo-local checks over global machine assumptions
4. Run `dart run tools/bin/agent_check.dart --rules-only` for quick structural feedback
5. Run the focused test file for the touched feature
6. Run full `dart run tools/bin/agent_check.dart` before handing over broad changes
